...
4. ~~CRA is entirely manual, end to end.~~ **Closed in Session 9** for the common case (`funding_source: PAID_SUBSCRIPTION`) — recording the subscription now auto-triggers the conversion, no second manual call needed. Still genuinely blocked on there being no payment gateway: `POST /finance/subscriptions` itself is still a human typing in that a payment arrived. Institutional/sponsored funding sources still require the manual `/convert` endpoint (kept for exactly this).
...
| VQ-B works | ✅ Built and verified live (session 8) | Gap #1 closed — see "Session 8" below. Targeting/rate-type model is PDR-09 (provisional, section 11 text unavailable this session) |
...
| Referral commission works | ✅ Tested (calc) / ✅ Automatic for PAID_SUBSCRIPTION, verified live (session 9) | Gap #4 closed for the common case — see "Session 9" below. Still genuinely blocked: no real payment gateway exists, so `POST /finance/subscriptions` recording the payment is itself still a human action; this closes the *second* manual step (converting the commission), not the first |
...
| Refunds/chargebacks can reverse commission | ✅ Automatic for CRA, verified live (session 7) | Scoped to referral commission only — see "Session 7" below. VCPM/BQE retroactive adjustment on refund is NOT covered (separate, larger problem) |
...
| Teacher dashboard works | ✅ Real UI shipped and verified live (session 10) | Gap #6 closed for the teacher side — `creator-earnings.html` rewired from a fictitious "40% commission" mock to the real ledger. See "Session 10" below |
| Admin controls work | ✅ API + payout-requests queue only, still no admin screens | Gap #6 partially open — a `GET /finance/payout-requests` listing was added (mirrors the fraud-flags/checkout-requests pattern), but campaigns/fraud-flags/ledger/contracts still have no admin UI, only the API endpoints from earlier sessions |
...
| Fraud foundations work | ✅ Gate mínimo implementado e verificado ao vivo (session 6) | Ver "Session 6" abaixo — não é fraude avançada (ML/scoring), é um gate explicável de concentração de IP/alunos antes de aprovar |
...
All Definition-of-Done rows are now ✅ except the ones explicitly blocked on external prerequisites (CRA full automation pending a payment gateway, UI) — see the updated table above. Fraud foundations closed in Session 6, refund reversal in Session 7, VQ-B in Session 8, below.

## Session 6 (2026-09-09) — minimal fraud gate before settlement approval

Closed gap #7 (blueprint had "Fraud foundations work ⚠️ Minimal", the last non-external-blocker item in the Definition of Done). Scope deliberately kept to explainable, auditable rule-based heuristics — not ML/scoring, which the policy document never asked for and would need far more historical data than exists today (zero approved settlement periods in production so far).

- `src/services/fraudDetection.ts` — pure `evaluateFraudSignals()`: below `FRAUD_MIN_SAMPLE_SIZE` (15) remunerable views, no signal (too little data to conclude anything, and a new teacher's first period shouldn't get stuck in review). Above that, two independent ratios: `LOW_IP_DIVERSITY` (< 15% distinct IPs per remunerable view — few devices replaying content) and `LOW_STUDENT_DIVERSITY` (< 25% distinct students per remunerable view — a handful of accounts driving all the "revenue"). Thresholds are provisional (PDR-08) — there's no real settlement history yet to calibrate against; revisit once some exists.
- `src/services/earningsAggregation.ts::gatherFraudMetrics` — joins `qualified_views` (VQ-R/VQ-B in period) to `video_watch_events.ip_address` via `session_token`, chunked at 200 tokens per query to stay under PostgREST's practical `.in()` limit.
- `src/services/settlementEngine.ts::runApprovePhase` — rewritten from a single bulk `UPDATE ... WHERE status='VALIDATING'` (all teachers at once) to a **per-teacher loop**: gathers metrics, evaluates signals, and only transitions a teacher's `VALIDATING` rows to `APPROVED` if clean. A flagged teacher's rows are simply left in `VALIDATING` (nothing is rejected or deleted) plus one `earnings_audit_log` row (`action='FRAUD_FLAG'`, metrics + signals in `after_state`). Unflagged teachers in the same period still auto-approve normally — the gate never blocks the whole batch.
- `GET /api/finance/fraud-flags` (`src/routes/finance.ts`) — lists flags with the teacher's name/email resolved (manual second query, since `earnings_audit_log.entity_id` is a generic column with no FK PostgREST can embed on). Review path for a flagged teacher is the existing `POST /api/finance/earnings/:id/approve` (no new endpoint needed — it already does exactly "approve this one ledger row manually").
- `tests/fraudDetection.test.ts` — 6 unit tests on the pure evaluator (sample-size floor, each signal individually, both together, healthy distribution, division-by-zero safety).

**Live-verified against production** (synthetic teacher/student/lesson data, created and deleted within the same run, same method as Session 5's concurrency tests): ran the real `POST /api/finance/earnings/close-period/approve` endpoint (not a mocked call) against two synthetic teachers in one period — one with 25 views from 25 distinct students across 20 IPs (clean), one with 25 views from only 2 students on 1 IP (should flag both signals). Result: the clean teacher's 2 ledger rows transitioned to `APPROVED`; the flagged teacher's rows stayed `VALIDATING`, with exactly one `FRAUD_FLAG` audit row showing both signals with correct percentages (`4.0%` IP ratio, `8.0%` student ratio); `GET /fraud-flags` correctly listed it with the teacher's real name/email resolved; `POST /earnings/:id/approve` correctly overrode the flag for one of the two rows on manual review. `npx vitest run` → 100/100, `npx vite build` clean.

## Session 7 (2026-09-09) — automatic referral commission reversal on refund

Closed "Refunds/chargebacks can reverse commission ⚠️ Manual only" — the last blueprint gap not blocked on the missing payment gateway. Deliberately scoped to CRA (referral commission) only: it's the one earning type tied 1:1 to a single purchase event. VCPM/BQE are tied to ongoing video-watch activity, not a purchase — retroactively unwinding those for a refunded period would mean walking back every `qualified_views` row's funding eligibility and every derived ledger entry, a much bigger problem the blueprint never asked this gap to solve.

- `database/migrations/035_referral_commission_auto_reversal.sql` — two changes: (1) `subscriptions.status` CHECK extended to add `'refunded'`, distinct from `'cancelled'` (a lapsed renewal isn't a refund — only `'refunded'` should ever trigger a reversal); (2) `referral_attributions.converted_subscription_id` (nullable FK to `subscriptions`), so the system knows **exactly** which subscription converted a given commission. Without that column, a student's *second* subscription being refunded could have wrongly reversed a commission that actually came from their *first* — `referral_attributions` only ever stores one row per student for life (Art. 21-24, "first click wins"), with no other link back to which purchase converted it. `POST /referrals/:studentId/convert` (`src/routes/finance.ts`) now accepts an optional `subscriptionId` in the body and stores it there.
- `POST /api/finance/subscriptions/:id/refund` (`src/routes/finance.ts`, next to the existing `/cancel`) — marks the subscription `refunded` (409 if already refunded — idempotent), then checks `referral_attributions` for that student: only if `converted_subscription_id` matches **this exact subscription id**, and only if no `REVERSAL` row already points at that commission (`reversal_of_id`), does it insert a `REVERSAL` ledger entry for `-1 ×` the original `gross_amount`, `status: APPROVED` immediately (the money is already gone, no reason to wait for another settlement cycle), `reversal_of_id` set. The original `REFERRAL_COMMISSION` row is never edited or deleted — same immutable-ledger rule as everywhere else in this domain. Every other subscription refund (no matching attribution, or an unrelated subscription of the same student) just marks `refunded` and reverses nothing — silence is the safe default here, not a guess.

**Live-verified against production** (synthetic teacher + student + two subscriptions + one already-converted `REFERRAL_COMMISSION` ledger row, created and deleted within the run): refunding the student's *second*, unrelated subscription correctly reversed nothing (`reversal: null` in the response). Refunding the *actual converting* subscription correctly created one `REVERSAL` row for exactly `-75.00` (matching the original `75.00`), leaving the teacher's net ledger balance at `0.00` for that pair, with `reversal_of_id` pointing at the original entry and the original untouched. A second refund call on the same subscription correctly returned `409 Subscrição já marcada como reembolsada` instead of double-reversing. `npx vitest run` → 100/100, `npx vite build` clean.

## Session 8 (2026-09-09) — VQ-B bonified-view campaigns

Closed gap #1, the last item in the Definition of Done. Important caveat: the actual policy text for section 11 (the implementation prompt's detailed VQ-B spec) is not available in this session — only the fragment already captured in this blueprint from an earlier reading (the three rate-type names, `NORMAL_VCPM`/`PERCENTAGE_OF_VCPM`/`FIXED_VCPM`). Everything else — campaign targeting model, exact PERCENTAGE_OF_VCPM math, whether VQ-B still counts toward the Art. 13 repeat limit — is **PDR-09, a provisional interpretation**, not a confirmed reading of the source document. Flagging this explicitly rather than presenting it as settled: if the real section 11 text turns out to specify something different (e.g. campaigns scoped by teacher rather than by lesson), the targeting model here would need revisiting, though the calculation math and DB plumbing would mostly carry over.

**Design decisions made (PDR-09):**
- A campaign covers a **set of specific lessons** (not a teacher, not the whole platform) — the most literal reading of "bónus de visualização em lote" (batch of lessons).
- `NORMAL_VCPM`: the view is classified `VQ-B` (for campaign reporting) but earns exactly what a `VQ-R` view would — implemented as `PERCENTAGE_OF_VCPM` at a fixed 100%, using the teacher's actual `VQ-R` count for the period as the "current tier" reference (`vcpmEngine.ts::marginalVcpmRate`).
- `PERCENTAGE_OF_VCPM`: `rate_value`% of the marginal VCPM rate the teacher's `VQ-R` count already reaches that period (e.g. 150.00 = 150%) — campaign views never affect the underlying `VQ-R` tier progression itself, they ride on top of it.
- `FIXED_VCPM`: `rate_value` MZN per 1,000 views, flat, ignoring tiers entirely.
- A `VQ-B` view still counts toward the Art. 13 repeat limit (2 remunerable views/student/lesson/30 days) — this was **already true before this session** (migration 030's repeat-limit query already summed `VQ-R` and `VQ-B` together); a campaign was never meant to be a way around that cap, and this session didn't change that behavior, just finally made it reachable.
- Per Art. 18-19 (already documented in `bqeEngine.ts` before this session — "o bónus incide só sobre a RCE (VCPM de VQ-R + VQ-B)"), the BQE quality bonus base is now `rceAmountMzn + bonifiedViewAmountMzn` combined, not `VQ-R` alone as it was calculating until this session — this was a **latent bug** the existing comment already flagged but the code never implemented; fixed as part of wiring VQ-B through.

**What shipped:**
- `database/migrations/036_vq_b_campaigns.sql` — `vq_b_campaigns` (name, rate_type, rate_value, starts_at/ends_at, active) + `vq_b_campaign_lessons` (junction, a campaign can cover several lessons) + `qualified_views.campaign_id` (nullable FK, set when a view is classified `VQ-B`). Also `CREATE OR REPLACE FUNCTION fn_record_watch_heartbeat` — same body as migration 030 except the one branch that used to always classify `VQ-R`: now checks for an active campaign covering `p_lesson_id` first. This was the highest-risk change of the session (replacing a function already live in production) — see live verification below.
- `src/services/vcpmEngine.ts` — `marginalVcpmRate()`, `calculatePercentageOfVcpmEarnings()`, `calculateFixedVcpmEarnings()`, all pure/BigInt-safe like the existing `calculateVcpmEarnings`. 11 new unit tests.
- `src/services/earningsAggregation.ts::estimateTeacherEarnings` — now also loads `campaign_id` per view, groups `VQ-B` views by campaign, computes each campaign's contribution by its `rate_type`, and sums into a new `bonifiedViewAmountMzn` (written to the ledger as a new `earning_type: 'BONIFIED_VIEW'` row — that value already existed in migration 030's `CHECK` constraint, unused until now). `settlementEngine.ts`'s `VALIDATING`→`APPROVED` transition updated to include `BONIFIED_VIEW` alongside `VCPM`/`QUALITY_BONUS` (it would otherwise have been silently stuck in `ESTIMATED` forever).
- `POST/GET/PATCH /api/finance/vq-b-campaigns` (`src/routes/finance.ts`) — admin CRUD, same `requireFinanceOrAdmin` pattern as ambassador-fees/special-content. `PATCH` can deactivate a campaign or reschedule it.

**Live-verified against production** (synthetic teacher + student + two lessons, one campaign, created and deleted within the run — all through the real deployed API, not mocks): created a `PERCENTAGE_OF_VCPM` 150% campaign on one lesson via `POST /vq-b-campaigns`; a real heartbeat on that lesson classified `VQ-B` (confirmed via the actual `POST /api/video/:lessonId/heartbeat` response); the **same call on an unrelated, non-campaign lesson still classified `VQ-R`** (regression check on the replaced production function — passed); `POST /earnings/estimate` for the period produced `vqRCount: 1, vqBCount: 1, rceAmountMzn: "0.10", bonifiedViewAmountMzn: "0.15"` — exactly `1 view × 150% × 100 MZN/1000` — with a `BONIFIED_VIEW` ledger row confirmed via `GET /earnings`; deactivating the campaign via `PATCH` and firing another heartbeat on the same lesson correctly fell back to `VQ-R`. `npx vitest run` → 111/111, `npx vite build` clean.

**Not covered by this session, left open:** platform-wide or teacher-wide campaign scopes (only lesson-scoped exists); a teacher/admin UI for creating campaigns (API only, same as the rest of this domain); true concurrency testing of the new campaign-lookup branch specifically (Session 5's concurrency suite predates this change — the existing advisory lock still covers it structurally, but wasn't re-run against this exact new code path).

## Session 9 (2026-09-10) — automatic referral commission conversion

Closed gap #4 for the common case. Same philosophy as Session 7's refund reversal: there is still no payment gateway, so "automatic" cannot mean "reacts to a real purchase event" — it means "doesn't require a *second* manual action after the first one." Today the only way a subscription exists at all is a human calling `POST /api/finance/subscriptions` to record a payment that already happened elsewhere (M-Pesa, bank transfer). Until this session, that same human then had to separately remember to call `POST /referrals/:studentId/convert` with the net revenue, or the referring teacher's commission silently never got created.

- Extracted the conversion logic (attribution lookup, `converted_at` guard, 30-day window check via `checkAttributionWindow`, commission calculation, ledger insert, attribution update) out of `POST /referrals/:studentId/convert` into a shared `attemptReferralConversion()` helper in `src/routes/finance.ts`. It never throws for "nothing to convert" — returns `{ converted: false, reason: 'NO_ATTRIBUTION' | 'ALREADY_CONVERTED' | 'WINDOW_EXPIRED' | 'SELF_REFERRAL' }` so callers decide whether that's an error (the manual endpoint, which maps each reason to its original HTTP status) or an unremarkable non-event (the new automatic trigger).
- `POST /api/finance/subscriptions` now calls `attemptReferralConversion()` right after inserting the subscription, **only** when `funding_source` is `PAID_SUBSCRIPTION` (the default) and `amount` is a positive number — for that case, `subscriptions.amount` is a reasonable stand-in for "net eligible revenue" per PDR-002. Institutional/sponsored/corporate funding sources are deliberately left to the manual endpoint, since "net eligible revenue" for those isn't simply the recorded amount and needs a human call. Never fails subscription creation if the conversion attempt errors — logged and swallowed, same as every other best-effort side-effect in this domain (e.g. `logEarningsAudit`). The outcome rides along in the response as `data.referralConversion`.
- `POST /referrals/:studentId/convert` (manual) is unchanged in behavior/API shape, just rebuilt on the shared helper — kept specifically for the institutional/sponsored cases and for correcting anything the automatic path missed.

**Live-verified against production** (synthetic teacher + two students — one with a referral attribution, one without — created and deleted within the run, all through the real deployed API): registering a `PAID_SUBSCRIPTION` for the referred student auto-converted correctly — response included `referralConversion: { converted: true, commissionMzn: "75.00" }` for a 500 MZN subscription (15% = 75.00, exact). Three regression/guard checks, all correct: registering a subscription for the *unrelated* student (no attribution) → `{ converted: false, reason: "NO_ATTRIBUTION" }`; a *second* subscription for the already-converted student → `{ converted: false, reason: "ALREADY_CONVERTED" }`; an `INSTITUTIONAL_PAID` subscription → `referralConversion: null` (never attempted). Confirmed via `GET /earnings` that exactly one `REFERRAL_COMMISSION` row existed for the teacher despite three subscription-creation calls — no double-conversion. Confirmed the manual `/convert` endpoint still works standalone (correct `404` for a student with no attribution at all). `npx vitest run` → 111/111, `npx vite build` clean.

**Still genuinely open (external, not code):** the payment gateway itself. Nothing here changes that `POST /finance/subscriptions` requires a human to already know a payment arrived — this session closed the gap *between* that recording and the commission being calculated, not the gap between a real purchase and the recording.

## Session 10 (2026-09-10) — teacher-facing earnings UI

Closed gap #6 for the teacher side. Important discovery made at the start of this session: `src/pages/creator-earnings.html` **already existed and already had a polished UI** (KPI cards, chart, donut, top-lessons table, withdrawal history, a withdraw modal) — but it was entirely wired to `GET/POST /api/creator/earnings*` in `creator.ts`, a completely different, pre-existing "40% teacher / 60% platform" revenue-share mockup with hardcoded fictitious lesson titles (`"Leis de Newton — Aplicações Práticas"`, etc.), a fake withdrawal history, and a `POST /earnings/withdraw` that never touched the database at all — just returned a fabricated reference. It predated the real Teacher Earnings V1 policy work and was never connected to it. So this gap wasn't "add a UI" — it was "notice the UI is lying, and rewire it to the real ledger."

- **Removed** `GET /api/creator/earnings` and `POST /api/creator/earnings/withdraw` from `creator.ts` entirely (dead/fictitious code, not kept behind a flag or commented out).
- **New teacher-scoped endpoints** in `src/routes/earnings.ts` (all `requireTeacher`, scoped to `user.id` — a teacher can only ever see their own data):
  - `GET /payable` — mirrors the admin `GET /finance/earnings/payable` query but restricted to the caller.
  - `GET /history` — flat ledger statement, most recent first.
  - `GET /periods?months=` — `VCPM`/`QUALITY_BONUS`/`BONIFIED_VIEW` totals grouped by settlement period (the only earning types that reliably carry `period_start`/`period_end`; `REFERRAL_COMMISSION` and the contract-based types are point-in-time events, shown only in `/history`).
  - `GET /top-lessons?periodStart&periodEnd` — groups the teacher's own `VQ-R`/`VQ-B` `qualified_views` by lesson, joined to lesson titles. Real per-lesson performance, not the old fake `views`/`completions`/`trend` columns (that data doesn't exist as a per-lesson breakdown anywhere in the real ledger).
  - `GET /payouts` — the teacher's own `teacher_payouts` history (real payments already made).
  - `POST /payout-request` — validates against the real approved balance, then writes a `PAYOUT_REQUESTED` row to `earnings_audit_log` (same audit mechanism as the fraud gate and refund reversal) with a generated reference. Deliberately does **not** touch `teacher_payouts` or mark anything `PAID` — same principle as every other payout-adjacent endpoint in this domain: no gateway exists, so this can only ever be a request for a human to act on, never an execution.
- **New admin endpoint** `GET /api/finance/payout-requests` (`src/routes/finance.ts`) — lists those requests with the teacher's name/email resolved, same pattern as `checkout-requests`/`fraud-flags`. This is the only admin-facing piece of UI-adjacent work this session; a dashboard screen to browse it doesn't exist yet (Admin controls row in the DoD table stays partially open).
- **`src/pages/creator-earnings.html` rewritten**: period-pill selector (7d/30d/90d/12m/all, meaningless against real monthly settlement periods) replaced with a simple "current month" label; 4 KPI cards now show real RCE / Bonificado (VQ-B) / Bónus Qualidade (BQE) / Saldo Aprovado instead of fake commission/students/completions/pending; the "alunos por plano" donut became a VQ-R/VQ-B/other classification donut (real data, `/summary`); the earnings trend chart now plots real per-period totals from `/periods`; top lessons and payment history both hit the new real endpoints; the withdraw modal posts to `/payout-request` instead of the old fake endpoint, checks against the real approved balance, and its copy no longer promises "SMS/email confirmation" or "3 business days" — it says a human on the finance team will process it, which is what actually happens.

**Live-verified against production** (synthetic teacher with 3 real `VQ-R` `qualified_views` this month, an `APPROVED` ledger balance from a past period, and one already-`PAID` payout, created and deleted within the run — every check below hit the real deployed API, not a mock): `GET /summary` → `vqRCount: 3, rceAmountMzn: "0.30"` for the live current-month estimate; `GET /payable` → `totalApprovedMzn: "640.00", willPay: true` (600+40 from the seeded past-period ledger, correctly separate from the current month's live estimate); `GET /periods` → two periods, `900` (the paid one) and `640` (the approved one), correctly grouped and summed; `GET /top-lessons` → the one seeded lesson with `qualifiedViews: 3`; `GET /payouts` → the one seeded real payout row. `POST /payout-request` for 600 MZN (within the 640 balance) succeeded with a generated reference; a second request for 9999 MZN correctly failed with `"Valor pedido (9999 MZN) excede o saldo aprovado disponível (640.00 MZN)"`; `GET /finance/payout-requests` (admin) correctly listed the first request with the teacher's real name/email resolved. Confirmed the old `GET /api/creator/earnings` now returns `404 Route not found`. Confirmed `creator-earnings.html` still redirects unauthenticated requests to `/login.html` (F7's session guard, from Session 6, still covers this page — untouched by this session's changes). `npx vitest run` → 111/111, `npx vite build` clean.

**Not covered by this session, left open:** any admin dashboard screen (campaigns, fraud-flags, the new payout-requests queue, contracts, ledger browser all remain API-only — this session only closed the *teacher* half of gap #6); a way for the teacher to see or cancel a payout request they already made (it's written to the audit log but nothing currently reads it back to them); recurring/scheduled UI refresh (the page loads once on `DOMContentLoaded`, no polling).
